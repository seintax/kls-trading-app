...
const helper = require('./statement.helper')
...
const byFilter = handler(async (req, res) => {
    ...
    const { beg, end, branch, category, id } = helper.fields
    let params = [p(param.fr).Exactly(), p(param.to).Exactly()]
    let clause = [f(beg).IsEqual(), f(end).IsEqual(), f(branch).CanBeNull(req.query.store), f(category).CanBeNull(req.query.category)]
    let series = [f(id).Asc()]
    let limits = undefined
    const builder = helper.inquiry(clause, params, series, limits)
    await poolarray(builder, (err, ans) => {
...
const byNoneRange = handler(async (req, res) => {
    const param = helper.parameters(req.query)
    const { branch, category, id } = helper.fields
    let params = []
    let clause = [f(branch).CanBeNull(req.query.store), f(category).CanBeNull(req.query.category)]
