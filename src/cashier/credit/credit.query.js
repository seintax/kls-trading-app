...
const helper = require('./credit.helper')
...
const byOngoing = handler(async (req, res) => {
    const param = helper.parameters(req.query)
    const { code, status, id } = helper.fields
    let params = [p(param.code).Contains(), "ON-GOING"]
    let clause = [f(code).Like(), f(status).IsEqual()]
    let series = [f(id).Asc()]
    let limits = undefined
    const builder = helper.inquiry(clause, params, series, limits)
    await poolarray(builder, (err, ans) => {
